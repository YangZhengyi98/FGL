...
from utility.load_data_userbatch import *
...
class fastlgcn(object):
    def __init__(self, sess, data_config, pretrain_data):
        # argument settings
        self.dataset = args.dataset
        self.model_type = 'fastlgcn'
        ...
        self.evaluator = ProxyEvaluator(data_config['train_items'], data_config['test_set'], negative_matrix=None)
        self.n_fold = 100
        ...
        self.beta = args.beta
        #self.n_user_per_batch = args.n_user_per_batch
        ...
        *********************************************************
        ...
        self.users_sampled = tf.placeholder(tf.int32, shape=(None,))
        self.n_this_batch = tf.placeholder(tf.float32)
        ...
        *********************************************************
        ...
        self.u_s_embeddings = tf.nn.embedding_lookup(self.ua_embeddings, self.users_sampled)
        self.pos_i_g_embeddings = tf.nn.embedding_lookup(self.ia_embeddings, self.pos_items)
       # self.neg_i_g_embeddings = tf.nn.embedding_lookup(self.ia_embeddings, self.neg_items)
        self.u_g_embeddings_pre = tf.nn.embedding_lookup(self.weights['user_embedding'], self.users)
        self.u_s_embeddings_pre = tf.nn.embedding_lookup(self.weights['user_embedding'], self.users_sampled)
        self.pos_i_g_embeddings_pre = tf.nn.embedding_lookup(self.weights['item_embedding'], self.pos_items)
        self.i_g_embeddings_pre = self.weights['item_embedding']
        ...
        *********************************************************
        ...
        if self.loss_func == 1:
            self.loss = self.creat_fast_loss(self.u_g_embeddings, self.pos_i_g_embeddings, self.u_s_embeddings ,self.ia_embeddings, self.alpha, self.beta)

        else:
            self.loss = self.creat_fast_loss2(self.u_g_embeddings, self.pos_i_g_embeddings, self.u_s_embeddings ,self.ia_embeddings, self.alpha)


        #self.loss = self.creat_fast_loss(self.u_g_embeddings, self.pos_i_g_embeddings, self.u_s_embeddings ,self.ia_embeddings, self.alpha)
    ...
    def create_model_str(self):
        ...
        log_dir += '/' + args.dataset + '/lr_' + str(self.lr) + '/alpha_' + str(self.alpha)
        return log_dir
    ...
    def creat_fast_loss(self, users, pos_items, users_sampled, items, alpha, beta):
        pos_scores = tf.reduce_sum(tf.multiply(users, pos_items), axis=1)
        pos_loss = tf.reduce_sum((alpha - 1) * tf.square(pos_scores) - 2 * alpha * beta * pos_scores)
        all_loss = tf.trace(tf.matmul(tf.transpose(users_sampled), tf.matmul(users_sampled, tf.matmul(tf.transpose(items), items))))
        #regularizer = tf.nn.l2_loss(self.u_s_embeddings_pre) / self.n_user_per_batch + tf.nn.l2_loss(self.i_g_embeddings_pre) / self.n_items
        #regularizer = tf.nn.l2_loss(self.u_g_embeddings_pre) / self.n_this_batch + tf.nn.l2_loss(self.pos_i_g_embeddings_pre) / self.n_this_batch
        regularizer = tf.nn.l2_loss(self.u_g_embeddings_pre) + tf.nn.l2_loss(self.pos_i_g_embeddings_pre)

        fast_loss = pos_loss + all_loss + self.decay * regularizer

        return fast_loss

    def creat_fast_loss2(self, users, pos_items, u, i, alpha):
        pos_scores = tf.reduce_sum(tf.multiply(users, pos_items), axis=1)
        pos_loss = tf.reduce_sum((alpha - 1) * tf.square(pos_scores) - 2 * alpha * pos_scores)
        #all_loss = tf.trace(tf.matmul(tf.transpose(users_sampled), tf.matmul(users_sampled, tf.matmul(tf.transpose(items), items))))
        n_u = self.n_users
        n_i = self.n_items
        n_emb = self.emb_dim
        temp_u1 = tf.tile(tf.reshape(u, [n_u, n_emb, -1]), [1, 1, n_emb])
        temp_u2 = tf.tile(tf.reshape(u, [n_u, -1, n_emb]), [1, n_emb, 1])
        temp_u = tf.reduce_sum(tf.multiply(temp_u1, temp_u2), axis=0)

        temp_i1 = tf.tile(tf.reshape(i, [n_i, n_emb, -1]), [1, 1, n_emb])
        temp_i2 = tf.tile(tf.reshape(i, [n_i, -1, n_emb]), [1, n_emb, 1])
        temp_i = tf.reduce_sum(tf.multiply(temp_i1, temp_i2), axis=0)

        all_loss = tf.reduce_sum(tf.multiply(temp_u, temp_i))
        regularizer = tf.nn.l2_loss(self.u_g_embeddings_pre) + tf.nn.l2_loss(self.pos_i_g_embeddings_pre)
        fast_loss = pos_loss + all_loss + self.decay * regularizer
        return fast_loss
...
if __name__ == '__main__':
    ...
    config = dict()
    data_generator = Data_userbatch(path=args.data_path + args.dataset, batch_size=args.batch_size, k=0)
    ...
    *********************************************************
    ...
    #model = fastlgcn(data_config=config, pretrain_data=pretrain_data)
    #print(model.dataset)
    #print('loss_func:', model.loss_func)
    ...
    *********************************************************
    ...
    model = fastlgcn(sess=sess, data_config=config, pretrain_data=pretrain_data)
    print(model.dataset)
    print('loss_func:', model.loss_func)
    print('Alpha:', model.alpha)
    print('Decay:', model.decay)
    print('learning rate:', model.lr)
    print('n_layers:', model.n_layers)
    print('emb_dim:', model.emb_dim)
    saver = tf.train.Saver()
    """
    *********************************************************
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        f.write(
            'embed_size=%d, lr=%.4f, layer_size=%s, keep_prob=%s, alpha=%s, loss_type=%s, adj_type=%s\n'
            % (args.embed_size, args.lr, args.layer_size, args.keep_prob, args.alpha, args.loss_type, args.adj_type))
    ...
    *********************************************************
    Train.
    """
    tensorboard_model_path = '/ghome/yangzy/fastlgcn/lgcn/tensorboard'
    ...
    for epoch in range(args.epoch):
        ...
        loss = 0.
        #n_batch = model.n_users // model.n_user_per_batch + 1
        n_batch = 1
        # loss_test, mf_loss_test, emb_loss_test, reg_loss_test = 0., 0., 0., 0.
        users_sampled, users, pos_items, n_this_batch = data_generator.sample_all()

        for idx in range(n_batch):
            #users_sampled, users, pos_items, n_this_batch = data_generator.sample_all()
            _, batch_loss= sess.run(
                [model.opt, model.loss],
                feed_dict={model.users: users, model.pos_items: pos_items, model.users_sampled: users_sampled, model.n_this_batch: n_this_batch})

            loss += batch_loss / len(users)
            # mf_loss += batch_mf_loss / n_batch
            # emb_loss += batch_emb_loss / n_batch
        summary_train_loss = sess.run(model.merged_train_loss,
                                      feed_dict={model.train_loss: loss})
        ...
        #            (epoch, loss, ret['recall'][0], ret['recall'][-1],
        ...
        #
        # if args.verbose > 0:
        #     perf_str = 'Epoch %d [%.1fs + %.1fs]: test, recall=[%.5f, %.5f], ' \
        #                'precision=[%.5f, %.5f], hit=[%.5f, %.5f], ndcg=[%.5f, %.5f]' % \
        #                (
        ...
        #                ret['ndcg'][0], ret['ndcg'][-1])
        #     print(perf_str)
        print(model.evaluator.metrics_info())

        print("epoch %d:\t%s" % (epoch, model.evaluate()))
